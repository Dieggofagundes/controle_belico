...
import { PELOTOES, MODELOS_ARMAMENTO, ACESSORIOS_PADRAO, ITENS_MATERIAL_VIATURA, ITENS_MUNICAO_ESPECIAL } from "../../types";
...
function handleFotoChange(index: number, file: File | null) {
  if (!file) {
    atualizarLinha(index, { foto_armamento: null });
    return;
  }
  const reader = new FileReader();
  reader.onload = () => {
    atualizarLinha(index, { foto_armamento: reader.result as string });
      };
...
<div className="field">
<label>Foto do Armamento</label>
<input
type="file"
accept="image/*"
onChange={(e) => handleFotoChange(i, e.target.files ? e.target.files[0] : null)}
/>
</div>
</div>

{linha.foto_armamento && (
<div style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
<img src={linha.foto_armamento} alt="Foto do armamento" style={{ width: 90, height: 90, objectFit: "cover", borderRadius: 6, border: "1px solid var(--color-line)" }} />
<button type="button" className="btn btn-danger" style={{ padding: "4px 10px", fontSize: 11 }} onClick={() => handleFotoChange(i, null)}>
Remover foto
</button>
</div>
)}
